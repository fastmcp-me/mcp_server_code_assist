...
async def test_read_file(test_repo):
    response = await process_instruction({"type": "read_file", "path": "test.txt"}, test_repo)
...
async def test_read_multiple(test_repo):
    ...
    response = await process_instruction({"type": "read_multiple", "paths": ["test.txt", "other.txt"]}, test_repo)
...
async def test_create_file(test_repo):
    response = await process_instruction({"type": "create_file", "path": "new.txt", "content": "new content"}, test_repo)
...
async def test_modify_file(test_repo):
    response = await process_instruction({"type": "modify_file", "path": "test.txt", "replacements": {"test": "modified"}}, test_repo)
...
async def test_rewrite_file(test_repo):
    response = await process_instruction({"type": "rewrite_file", "path": "test.txt", "content": "rewritten"}, test_repo)
    assert response["result"].startswith("--- original")
...
async def test_delete_file(test_repo):
    response = await process_instruction({"type": "delete_file", "path": "test.txt"}, test_repo)
